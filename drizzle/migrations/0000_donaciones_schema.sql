...
CREATE INDEX donaciones_busqueda_trgm ON public.donaciones USING gin (busqueda gin_trgm_ops);
CREATE INDEX donaciones_dni_idx ON public.donaciones (dni_ce);
CREATE INDEX donaciones_fecha_idx ON public.donaciones (fecha_recepcion);
CREATE INDEX donaciones_depto_idx ON public.donaciones (departamento);
CREATE INDEX donaciones_ciudad_idx ON public.donaciones (ciudad);
CREATE INDEX donaciones_created_idx ON public.donaciones (created_at DESC);
CREATE INDEX donaciones_wa_idx ON public.donaciones (whatsapp_message_id);
...
CREATE INDEX wa_events_pending ON public.whatsapp_webhook_events (received_at) WHERE processed_at IS NULL;
