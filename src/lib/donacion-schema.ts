...
const imgPng = (label: string) =>
  z
    .string()
    .max(500_000, { message: `${label} demasiado grande` })
    .regex(/^data:image\/png;base64,[A-Za-z0-9+/]+={0,2}$/, {
      message: `Debe registrar su ${label.toLowerCase()}`,
    });
